...
class RotateStandard(Operation):
    ...
    def __str__(self):
        ...
        return self.__class__.__name__+str(self.max_left_rotation)
...
class RotateRange(Operation):
    ...
    def __str__ (self):
        return self.__class__.__name__+str(self.max_right_rotation)
...
class Flip(Operation):
    ...
    def __str__( self ):
        return self.__class__.__name__+str(self.top_bottom_left_right)
...
class Distort(Operation):
    ...
    def __str__(self):
        return self.__class__.__name__+str(self.grid_width)+str(self.grid_height)+str(self.magnitude)
...
class RandomErasing(Operation):
    ...
    def __str__ (self):
        ...
        return self.__class__.__name__+str(self.rectangle_area)
...
class Blur(Operation):
    """
    Blurs the image in three different modes , average blur , gaussian blur and median blur
    """
    AVERAGE = 'average'
    GAUSSIAN = 'gaussian'
    MEDIAN = 'median'

    def __init__(self,probability, type_, radius = 1 ):

        """
        probability : 0-1
        radius : the number of pixels in each direction
        types are median average and gaussian
        """

        Operation.__init__(self,probability)
        self.radius = radius
        if type_ == Blur.AVERAGE or type_ == Blur.GAUSSIAN or type_ == Blur.MEDIAN:
            self.type_ = type_
        else :

            assert True , "unexpected type" + str(type)
    ...
    def perform_operation(self, images):
        ...
        for image in images:
            ...
            if self.type_ == Blur.AVERAGE:
                

                #augmented_images.append(ImageFilter.BoxBlur(self.radius).filter(image))
                augmented_images.append(image.filter(ImageFilter.BoxBlur(self.radius)))
            elif self.type_ == Blur.GAUSSIAN: 

                augmented_images.append(image.filter(ImageFilter.GaussianBlur(self.radius)))
            
            else :
                # radius is squared as the kernel size in pixels so 3 becomes 9 pixels
                augmented_images.append(image.filter(ImageFilter.MedianFilter(self.radius*self.radius)))

        return augmented_images
    
    def __str__(self):

        return self.__class__.__name__+self.type_+str(self.radius)
    

class Sharpen (Operation):

    def __init__(self,propability,min_factor,max_factor):
        """
        sharpens a given image 
        propability: 0.0 - 1.0 
        min_factor , max_factor : 0.0 - 2.0
        0.0 gives blurred image
        1.0 gives the original image
        2.0 gives sharp image

        """
        Operation.__init__(self,propability)
        self.min_factor = min_factor
        self.max_factor = max_factor
